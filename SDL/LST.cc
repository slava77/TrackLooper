...
namespace {
  ...
  void loadMaps(SDL::MapPLStoLayer& pLStoLayer) {
    ...
    auto endcap_geom = get_absolute_path_after_check_file_exists(trackLooperDir() +
                                                                 "/data/endcap_orientation_data_CMSSW_12_2_0_pre2.txt");
    auto tilted_geom = get_absolute_path_after_check_file_exists(trackLooperDir() +
                                                                 "/data/tilted_orientation_data_CMSSW_12_2_0_pre2.txt");
    ...
    auto mappath = get_absolute_path_after_check_file_exists(
        trackLooperDir() + "/data/module_connection_tracing_CMSSW_12_2_0_pre2_merged.txt");
    SDL::Globals<SDL::Dev>::moduleConnectionMap.load(mappath.c_str());

    auto pLSMapDir = trackLooperDir() + "/data/pixelmaps_CMSSW_12_2_0_pre2_0p8minPt/pLS_map";
...
void SDL::LST<SDL::Acc>::loadAndFillES(alpaka::QueueCpuBlocking& queue, struct modulesBuffer<alpaka::DevCpu>* modules) {
  ...
  auto path = get_absolute_path_after_check_file_exists(trackLooperDir() + "/data/centroid_CMSSW_12_2_0_pre2.txt");
